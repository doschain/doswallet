...
func (s *RPCSyncer) handleNotifications(ctx context.Context) error {
	...
	for {
		...
		case n, ok := <-c:
			...
			switch n := n.(type) {
			case blockConnected:
				op = "dosd.jsonrpc.blockconnected"
			...
			case relevantTxAccepted:
				op = "dosd.jsonrpc.relevanttxaccepted"
				nonFatal = true
				var rpt *chainhash.Hash
				rpt, err = s.wallet.RescanPoint()
				if err != nil || rpt != nil {
...
func (s *RPCSyncer) startupSync(ctx context.Context) error {
	...
	if err != nil {
		const op errors.Op = "dosd.jsonrpc.notifyblocks"
		return errors.E(op, err)
	}
	...
	for {
		...
		err := ctxdo(ctx, "dosd.jsonrpc.getheaders", func() error {
	...
	if err != nil {
		const op errors.Op = "dosd.jsonrpc.rebroadcastwinners"
		return errors.E(op, err)
	}
	_, err = s.rpcClient.RawRequest("rebroadcastmissed", nil)
	if err != nil {
		const op errors.Op = "dosd.jsonrpc.rebroadcastmissed"
		return errors.E(op, err)
	}
