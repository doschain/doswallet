...
func (b *rpcBackend) GetBlocks(ctx context.Context, blockHashes []*chainhash.Hash) ([]*wire.MsgBlock, error) {
	const op errors.Op = "dosd.jsonrpc.getblock"
...
func (b *rpcBackend) GetCFilters(ctx context.Context, blockHashes []*chainhash.Hash) ([]*gcs.Filter, error) {
	const opf = "dosd.jsonrpc.getcfilter(%v)"
...
func (b *rpcBackend) GetHeaders(ctx context.Context, blockLocators []*chainhash.Hash, hashStop *chainhash.Hash) ([]*wire.BlockHeader, error) {
	const op errors.Op = "dosd.jsonrpc.getheaders"
...
func (b *rpcBackend) LoadTxFilter(ctx context.Context, reload bool, addrs []dosutil.Address, outpoints []wire.OutPoint) error {
	const op errors.Op = "dosd.jsonrpc.loadtxfilter"
...
func (b *rpcBackend) PublishTransactions(ctx context.Context, txs ...*wire.MsgTx) error {
	const op errors.Op = "dosd.jsonrpc.sendrawtransaction"
...
func (b *rpcBackend) Rescan(ctx context.Context, blocks []chainhash.Hash, r wallet.RescanSaver) error {
	const op errors.Op = "dosd.jsonrpc.rescan"
...
func (b *rpcBackend) StakeDifficulty(ctx context.Context) (dosutil.Amount, error) {
	const op errors.Op = "dosd.jsonrpc.getstakedifficulty"

	r, err := b.rpcClient.GetStakeDifficulty()
	if err != nil {
		return 0, errors.E(op, err)
	}
	amount, err := dosutil.NewAmount(r.NextStakeDifficulty)
	if err != nil {
		return 0, errors.E(op, err)
	}
	return amount, nil
