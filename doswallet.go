...
import (
	...
	"gogs.doschain.org/doschain/doswallet/internal/prompt"
	"gogs.doschain.org/doschain/doswallet/internal/zero"
	ldr "gogs.doschain.org/doschain/doswallet/loader"
	"gogs.doschain.org/doschain/doswallet/p2p"
	"gogs.doschain.org/doschain/doswallet/rpc/legacyrpc"
	"gogs.doschain.org/doschain/doswallet/rpc/rpcserver"
...
func run(ctx context.Context) error {
	...
	loader := ldr.NewLoader(activeNet.Params, dbDir, cfg.GapLimit, cfg.AllowHighFees, cfg.RelayFee.ToCoin(), cfg.AccountGapLimit, cfg.DisallowChangeAddress)
...
func startChainRPC(ctx context.Context, certs []byte) (*chain.RPCClient, error) {
	...
	rpcc, err := chain.NewRPCClientConfig(activeNet.Params, &dcrrpcclient.ConnConfig{
		Host:                 cfg.RPCConnect,
		Endpoint:             "ws",
		User:                 cfg.DosdUsername,
		Pass:                 cfg.DosdPassword,
		Certificates:         certs,
