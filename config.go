...
import (
	...
	"gogs.doschain.org/doschain/doswallet/version"
...
const (
	defaultCAFilename       = "dosd.cert"
	defaultConfigFilename   = "doswallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "doswallet.log"
...
var (
	dcrdDefaultCAFile  = filepath.Join(dosutil.AppDataDir("dosd", false), "rpc.cert")
	defaultAppDataDir  = dosutil.AppDataDir("doswallet", false)
...
type config struct {
	...
	DisallowChangeAddress bool                `long:"disallowchangeaddress" description:"Do not use change address"`
	...
	DosdUsername     string                  `long:"dosdusername" description:"Username for dcrd authentication"`
	DosdPassword     string                  `long:"dosdpassword" default-mask:"-" description:"Password for dcrd authentication"`
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DosdUsername == "" {
		cfg.DosdUsername = cfg.Username
	}
	if cfg.DosdPassword == "" {
		cfg.DosdPassword = cfg.Password
	}
