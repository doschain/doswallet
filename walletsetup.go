...
func createWallet(ctx context.Context, cfg *config) error {
	...
	loader := loader.NewLoader(activeNet.Params, dbDir,
		cfg.GapLimit, cfg.AllowHighFees, cfg.RelayFee.ToCoin(), cfg.AccountGapLimit, cfg.DisallowChangeAddress)

	var privPass, pubPass, seed []byte
	var upgradeSLIP44 bool
	var err error
	c := make(chan struct{}, 1)
	go func() {
		reader := bufio.NewReader(os.Stdin)
		privPass, pubPass, seed, upgradeSLIP44, err = prompt.Setup(reader,
