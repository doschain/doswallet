...
func (s *Server) handlerClosure(ctx context.Context, request *dosjson.Request) lazyHandler {
...
func (s *Server) invalidAuth(req *dosjson.Request) bool {
	cmd, err := dosjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*dosjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := dosjson.MarshalResponse(req.Jsonrpc, req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := dosjson.MarshalResponse(req.Jsonrpc, req.ID, nil, dosjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response to client %s: %v",
				r.RemoteAddr, err)
	...
	// Marshal and send.
	mresp, err := dosjson.MarshalResponse(req.Jsonrpc, req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response to client %s: %v",
			r.RemoteAddr, err)
