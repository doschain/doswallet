...
//
//   https://gogs.doschain.org/doschain/doswallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://gogs.doschain.org/doschain/doswallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"gogs.doschain.org/doschain/dosd/addrmgr"
	...
	"gogs.doschain.org/doschain/doswallet/internal/cfgutil"
	h "gogs.doschain.org/doschain/doswallet/internal/helpers"
	"gogs.doschain.org/doschain/doswallet/internal/zero"
	"gogs.doschain.org/doschain/doswallet/loader"
	"gogs.doschain.org/doschain/doswallet/netparams"
	"gogs.doschain.org/doschain/doswallet/p2p"
	pb "gogs.doschain.org/doschain/doswallet/rpc/walletrpc"
	"gogs.doschain.org/doschain/doswallet/spv"
	...
	"gogs.doschain.org/doschain/doswallet/walletseed"
)
...
// dosutil.DecodeAddress, which does not perform the network check.
func decodeAddress(a string, params *chaincfg.Params) (dosutil.Address, error) {
	addr, err := dosutil.DecodeAddress(a)
	if err != nil {
...
func (s *walletServer) NextAddress(ctx context.Context, req *pb.NextAddressRequest) (
	*pb.NextAddressResponse, error) {
	...
	pubKeyAddr, err := dosutil.NewAddressSecpPubKey(pubKey.Serialize(), s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := dosutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument,
...
func (s *walletServer) ImportScript(ctx context.Context,
	...
	p2sh, err := dosutil.NewAddressScriptHash(req.Script, s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) SweepAccount(ctx context.Context, req *pb.SweepAccountRequest) (*pb.SweepAccountResponse, error) {
	...
	if req.FeePerKb > 0 {
		var err error
		feePerKb, err = dosutil.NewAmount(req.FeePerKb)
		if err != nil {
...
func (s *walletServer) BlockInfo(ctx context.Context, req *pb.BlockInfoRequest) (*pb.BlockInfoResponse, error) {
	...
	return &pb.BlockInfoResponse{
		...
		ApprovesParent:   header.VoteBits&dosutil.BlockValid != 0,
	}, nil
}
...
func (s *walletServer) UnspentOutputs(req *pb.UnspentOutputsRequest, svr pb.WalletService_UnspentOutputsServer) error {
	...
	inputDetail, err := s.wallet.SelectInputs(dosutil.Amount(req.TargetAmount), policy)
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	if req.IncludeChangeScript && inputDetail.Amount > dosutil.Amount(req.TargetAmount) {
...
func (s *walletServer) ConstructTransaction(ctx context.Context, req *pb.ConstructTransactionRequest) (
	*pb.ConstructTransactionResponse, error) {
	...
	if req.FeePerKb != 0 {
		feePerKb = dosutil.Amount(req.FeePerKb)
	}
...
func (s *walletServer) ValidateAddress(ctx context.Context, req *pb.ValidateAddressRequest) (*pb.ValidateAddressResponse, error) {
	...
	case udb.ManagedPubKeyAddress:
		...
		pubKeyAddr, err := dosutil.NewAddressSecpPubKey(result.PubKey,
...
func marshalBlock(v *wallet.Block) *pb.BlockDetails {
	...
	return &pb.BlockDetails{
		...
		ApprovesParent: v.Header.VoteBits&dosutil.BlockValid != 0,
		Transactions:   txs,
	}
}
...
func (s *messageVerificationServer) VerifyMessage(ctx context.Context, req *pb.VerifyMessageRequest) (
	*pb.VerifyMessageResponse, error) {
	...
	addr, err := dosutil.DecodeAddress(req.Address)
	if err != nil {
		return nil, translateError(err)
	}
