...
func (w *Wallet) nextAddress(op errors.Op, persist persistReturnedChildFunc, account, branch uint32,
	callOpts ...NextAddressCallOption) (dosutil.Address, error) {
...
func (w *Wallet) watchFutureAddresses(dbtx walletdb.ReadTx) error {
	...
	for account, a := range w.addressBuffers {
		...
		addrs := make([]dosutil.Address, 0, totalAddrs)
...
func (w *Wallet) NewExternalAddress(account uint32, callOpts ...NextAddressCallOption) (dosutil.Address, error) {
...
func (w *Wallet) NewInternalAddress(account uint32, callOpts ...NextAddressCallOption) (dosutil.Address, error) {
	const op errors.Op = "wallet.NewExternalAddress"
	return w.nextAddress(op, w.persistReturnedChild(nil), account, udb.InternalBranch, callOpts...)
}

func (w *Wallet) newChangeAddress(op errors.Op, persist persistReturnedChildFunc, account uint32) (dosutil.Address, error) {
...
// policy.
func (w *Wallet) NewChangeAddress(account uint32) (dosutil.Address, error) {
...
func (w *Wallet) AccountBranchAddressRange(account, branch, start, end uint32) ([]dosutil.Address, error) {
...
type P2PKHMasterSource struct {
	account uint32
	wallet  *Wallet
}

func (src *P2PKHMasterSource) Script() ([]byte, uint16, error) {
	addresses, err := src.wallet.AccountBranchAddressRange(src.account, udb.ExternalBranch, 0, 1)
	if err != nil {
		return nil, 0, err
	}
	script, err := txscript.PayToAddrScript(addresses[0])
	if err != nil {
		return nil, 0, err
	}
...
func (src *P2PKHMasterSource) ScriptSize() int {
...
func deriveChildAddresses(key *hdkeychain.ExtendedKey, startIndex, count uint32, params *chaincfg.Params) ([]dosutil.Address, error) {
	addresses := make([]dosutil.Address, 0, count)
...
func deriveChildAddress(key *hdkeychain.ExtendedKey, child uint32, params *chaincfg.Params) (dosutil.Address, error) {
...
func appendChildAddrsRange(addrs *[]dosutil.Address, key *hdkeychain.ExtendedKey,
