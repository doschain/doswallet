...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the dosutil.Tx of the read MsgTx and set its index.
		tx := dosutil.NewTx(msgTx)
		tx.SetIndex(dosutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: dosutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
