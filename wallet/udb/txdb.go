...
func fetchMinedBalance(ns walletdb.ReadBucket) (dosutil.Amount, error) {
	v := ns.Get(rootMinedBalance)
	if len(v) != 8 {
		return 0, errors.E(errors.IO, errors.Errorf("mined balance len %d", len(v)))
	}
	return dosutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt dosutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (dosutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (dosutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return dosutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (dosutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return dosutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (dosutil.Amount, error) {
	...
	return dosutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (dosutil.Amount, error) {
	...
	return dosutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = dosutil.Amount(byteOrder.Uint64(it.cv))
...
func valueDebit(amount dosutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount dosutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) dosutil.Amount {
	return dosutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func valueUnminedCredit(amount dosutil.Amount, change bool, opCode uint8,
...
func fetchRawUnminedCreditAmount(v []byte) (dosutil.Amount, error) {
...
func fetchRawUnminedCreditAmountChange(v []byte) (dosutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		return 0, false, errors.E(errors.IO, errors.Errorf("unmined credit len %d", len(v)))
	}
	amt := dosutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount dosutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = dosutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) dosutil.Amount {
	return dosutil.Amount(byteOrder.Uint64(v[59:67]))
}
