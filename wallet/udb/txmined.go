...
import (
	...
	"gogs.doschain.org/doschain/dosd/gcs/blockcf"
...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, dosutil.Address) (uint32, error)
}
...
func stakeValidate(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for i := range blockRec.transactions {
		...
		for i, output := range txRec.MsgTx.TxOut {
			...
			minedBalance += dosutil.Amount(output.Value)
		}
...
func (s *Store) AddCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     dosutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if block == nil {
		...
		v := valueUnminedCredit(dosutil.Amount(rec.MsgTx.TxOut[index].Value),
	...
	txOutAmt := dosutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) AddMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32) error {
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		dosutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func approvesParent(voteBits uint16) bool {
	return dosutil.IsFlagSet16(voteBits, dosutil.BlockValid)
}
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket, addr dosutil.Address) ([]*MultisigCredit, error) {
...
// without error.
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket, needed dosutil.Amount, syncHeight int32, minConf int32, all bool, account uint32) ([]*Credit, error) {
	var eligible []*minimalCredit
	var toUse []*minimalCredit
	var unspent []*Credit
	found := dosutil.Amount(0)
...
type InputSource struct {
	source func(dosutil.Amount) (*txauthor.InputDetail, error)
}
...
func (s *InputSource) SelectInputs(target dosutil.Amount) (*txauthor.InputDetail, error) {
...
func (s *Store) MakeInputSource(ns, addrmgrNs walletdb.ReadBucket, account uint32, minConf, syncHeight int32) InputSource {
	...
	var (
		currentTotal      dosutil.Amount
		currentInputs     []*wire.TxIn
		currentScripts    [][]byte
		redeemScriptSizes []int
	)

	f := func(target dosutil.Amount) (*txauthor.InputDetail, error) {
...
func (s *Store) balanceFullScan(ns, addrmgrNs walletdb.ReadBucket, minConf int32, syncHeight int32) (map[uint32]*Balances, error) {
	...
	for k, v := c.First(); k != nil; k, v = c.Next() {
		...
		case txscript.OP_SSTX:
			...
			votingAuthorityAmt := dosutil.Amount(0)
			lockedByTicketsAmt := dosutil.Amount(0)
			...
			totalInputAmount := dosutil.Amount(0)
			...
			for _, txin := range rec.MsgTx.TxIn {
				...
				if rawUnmined != nil {
					...
					if int(txin.PreviousOutPoint.Index) < len(tx.TxOut) {
						totalInputAmount += dosutil.Amount(tx.TxOut[txin.PreviousOutPoint.Index].Value)
					}
				} else {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards dosutil.Amount
	ImmatureStakeGeneration dosutil.Amount
	LockedByTickets         dosutil.Amount
	Spendable               dosutil.Amount
	Total                   dosutil.Amount
	VotingAuthority         dosutil.Amount
	Unconfirmed             dosutil.Amount
}
...
func (s *Store) TotalInput(dbtx walletdb.ReadTx, tx *wire.MsgTx) (dosutil.Amount, error) {
	ns := dbtx.ReadBucket(wtxmgrBucketKey)

	var total dosutil.Amount
	for _, in := range tx.TxIn {
		...
		total += dosutil.Amount(tx.TxOut[in.PreviousOutPoint.Index].Value)
	}
