...
type ManagedAddress interface {
	...
	// Address returns a dosutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *dosutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the dosutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() dosutil.Address {
	return a.address
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey chainec.PublicKey, compressed bool) (*managedAddress, error) {
	...
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = dosutil.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = dosutil.Hash160(pubKey.SerializeUncompressed())
	}
	address, err := dosutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams,
		dosec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *dosutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() dosutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := dosutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
