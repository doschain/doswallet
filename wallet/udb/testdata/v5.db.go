...
import (
	...
	_ "gogs.doschain.org/doschain/doswallet/wallet/internal/bdb"
...
func setup() error {
	...
	return walletdb.Update(db, func(dbtx walletdb.ReadWriteTx) error {
		...
		wif, err := dosutil.NewWIF(privKey, chainParams, chainec.ECTypeSecp256k1)
		if err != nil {
			return err
		}
...
func createUnsignedTicketPurchase(prevOut *wire.OutPoint,
	inputAmount, ticketPrice dosutil.Amount) (*wire.MsgTx, error) {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addr,
		dosutil.Amount(amountsCommitted[0]), ticketFeeLimits)
	if err != nil {
		return nil, err
	}
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB dosutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if dosutil.Amount(output.Value) > feeEstimate {
			amount := dosutil.Amount(output.Value) - feeEstimate
