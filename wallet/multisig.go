...
// from them.
func (w *Wallet) MakeSecp256k1MultiSigScript(secp256k1Addrs []dosutil.Address, nRequired int) ([]byte, error) {
	const op errors.Op = "wallet.MakeSecp256k1MultiSigScript"

	secp256k1PubKeys := make([]*dosutil.AddressSecpPubKey, len(secp256k1Addrs))
	...
	for i, addr := range secp256k1Addrs {
		...
		case *dosutil.AddressPubKeyHash:
			if addr.DSA(w.chainParams) != dosec.STEcdsaSecp256k1 {
				return nil, errors.E(op, errors.Invalid, "address key is not secp256k1")
			}
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*dosutil.AddressScriptHash, error) {
	const op errors.Op = "wallet.ImportP2SHRedeemScript"

	var p2shAddr *dosutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if errors.Is(errors.Exist, err) {
				...
				p2shAddr, _ = dosutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*dosutil.AddressScriptHash)
		return nil
	})
	if err != nil {
...
func (w *Wallet) FetchP2SHMultiSigOutput(outPoint *wire.OutPoint) (*P2SHMultiSigOutput, error) {
	...
	p2shAddr, err := dosutil.NewAddressScriptHashFromHash(
