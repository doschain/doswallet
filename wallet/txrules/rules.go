...
const DefaultRelayFeePerKb dosutil.Amount = 1e4

// IsDustAmount determines whether a transaction output value and script length would
// cause the output to be considered dust.  Transactions with dust outputs are
// not standard and are rejected by mempools with default policies.
func IsDustAmount(amount dosutil.Amount, scriptSize int, relayFeePerKb dosutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb dosutil.Amount) bool {
	...
	return IsDustAmount(dosutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
func CheckOutput(output *wire.TxOut, relayFeePerKb dosutil.Amount) error {
	if output.Value < 0 {
		return errors.E(errors.Invalid, "transaction output amount is negative")
	}
	if output.Value > dosutil.MaxAmount {
...
func FeeForSerializeSize(relayFeePerKb dosutil.Amount, txSerializeSize int) dosutil.Amount {
	fee := relayFeePerKb * dosutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > dosutil.MaxAmount {
		fee = dosutil.MaxAmount
	}
...
func PaysHighFees(totalInput dosutil.Amount, tx *wire.MsgTx) bool {
