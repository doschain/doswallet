...
// in DCP0001.
func (w *Wallet) nextRequiredDCP0001PoSDifficulty(dbtx walletdb.ReadTx, curHeader *wire.BlockHeader, chain []*BlockNode) (dosutil.Amount, error) {
	...
	if nextHeight < stakeDiffStartHeight {
		return dosutil.Amount(w.chainParams.MinimumStakeDiff), nil
	}
	...
	if nextHeight%intervalSize != 0 {
		return dosutil.Amount(curDiff), nil
	}
	...
	return dosutil.Amount(sdiff), nil
}
...
// trusted full node.
func (w *Wallet) NextStakeDifficulty() (dosutil.Amount, error) {
	const op errors.Op = "wallet.NextStakeDifficulty"
	var sdiff dosutil.Amount
...
func (w *Wallet) NextStakeDifficultyAfterHeader(h *wire.BlockHeader) (dosutil.Amount, error) {
...
func (w *Wallet) validateHeaderChainDifficulties(dbtx walletdb.ReadTx, chain []*BlockNode, idx int) ([]*BlockNode, error) {
	...
	for ; idx < len(chain); idx++ {
		...
		if deployments.DCP0001.Active(int32(h.Height), w.chainParams) {
			...
			if dosutil.Amount(h.SBits) != sdiff {
				err := errors.Errorf("%v has invalid PoS difficulty, got %v, want %v",
					&hash, dosutil.Amount(h.SBits), sdiff)
				return chain[idx:], errors.E(op, errors.Consensus, err)
			}
		}
