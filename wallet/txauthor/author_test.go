...
func p2pkhOutputs(amounts ...dosutil.Amount) []*wire.TxOut {
...
func TestNewUnsignedTransaction(t *testing.T) {
	...
	for i, test := range tests {
		...
		if tx.ChangeIndex < 0 {
			...
			changeAmount := dosutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
			if test.ChangeAmount == 0 {
