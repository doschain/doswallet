...
import (
	...
	"gogs.doschain.org/doschain/dosd/dosec/secp256k1"
	...
	dcrrpcclient "gogs.doschain.org/doschain/dosd/rpcclient"
	"gogs.doschain.org/doschain/dosd/txscript"
	"gogs.doschain.org/doschain/dosd/wire"
	"gogs.doschain.org/doschain/doswallet/deployments"
...
var (
	...
	wstxmgrNamespaceKey   = []byte("wstxmgr")
...
type Wallet struct {
	...
	balanceToMaintain dosutil.Amount
	subsidyCache      *blockchain.SubsidyCache

	// Start up flags/settings
	gapLimit              int
	accountGapLimit       int
	disallowChangeAddress bool
...
func (w *Wallet) BalanceToMaintain() dosutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance dosutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() dosutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee dosutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) loadActiveAddrs(ctx context.Context, dbtx walletdb.ReadTx, nb NetworkBackend) (uint64, error) {
	...
	loadBranchAddrs := func(branchKey *hdkeychain.ExtendedKey, n uint32, errs chan<- error) {
		...
		for child := uint32(0); child <= n; child += step {
			child := child
			g.Go(func() error {
				addrs := make([]dosutil.Address, 0, step)
...
func (w *Wallet) CommittedTickets(tickets []*chainhash.Hash) ([]*chainhash.Hash, []dosutil.Address, error) {
	const op errors.Op = "wallet.CommittedTickets"
	hashes := make([]*chainhash.Hash, 0, len(tickets))
	addresses := make([]dosutil.Address, 0, len(tickets))
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for _, v := range tickets {
			...
			var bestAddr dosutil.Address
			var bestAmount dosutil.Amount
			...
			for i := 1; i < len(tx.TxOut); i += 2 {
				...
				if _, ok := addr.(*dosutil.AddressPubKeyHash); !ok {
...
type (
	...
	createMultisigTxRequest struct {
		...
		pubkeys   []*dosutil.AddressSecpPubKey
		nrequired int8
		minconf   int32
	...
	for {
		...
		case txr := <-w.createTxRequests:
			...
			tx, err := w.txToOutputs("wallet.SendOutputs", txr.outputs,
				txr.account, txr.minconf, true, txr.inputSource, txr.changeSource)
			heldUnlock.release()
...
func (w *Wallet) Consolidate(inputs int, account uint32,
	address dosutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount dosutil.Amount, pubkeys []*dosutil.AddressSecpPubKey, nrequired int8, minconf int32) (*CreatedTx, dosutil.Address, []byte, error) {
...
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (dosutil.Address, error) {
...
func (w *Wallet) PubKeyForAddress(a dosutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr dosutil.Address) (sig []byte, err error) {
...
func VerifyMessage(msg string, addr dosutil.Address, sig []byte) (bool, error) {
	...
	recoveredAddr, err := dosutil.NewAddressSecpPubKey(serializedPK, addr.Net())
	if err != nil {
		return false, errors.E(op, err)
	}
...
func (w *Wallet) HaveAddress(a dosutil.Address) (bool, error) {
...
func (w *Wallet) AccountOfAddress(a dosutil.Address) (uint32, error) {
...
func (w *Wallet) AddressInfo(a dosutil.Address) (udb.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *udb.TxDetails, addrMgr *udb.Manager, syncHeight int32, net *chaincfg.Params) (sends, receives []dosjson.ListTransactionsResult) {
	addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
	...
	txTypeStr := dosjson.LTTTRegular
	switch details.TxType {
	case stake.TxTypeSStx:
		txTypeStr = dosjson.LTTTTicket
	case stake.TxTypeSSGen:
		txTypeStr = dosjson.LTTTVote
	case stake.TxTypeSSRtx:
		txTypeStr = dosjson.LTTTRevocation
	}

	// Fee can only be determined if every input is a debit.
	var feeF64 float64
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal dosutil.Amount
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := dosutil.Amount(output.Value).ToCoin()
		result := dosjson.ListTransactionsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   BlockIndex
			//
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]dosjson.ListTransactionsResult, error) {
	const op errors.Op = "wallet.ListSinceBlock"
	txList := []dosjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]dosjson.ListTransactionsResult, error) {
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]dosjson.ListTransactionsResult, error) {
...
// replies.
func (w *Wallet) ListAllTransactions() ([]dosjson.ListTransactionsResult, error) {
...
// transaction.
func (w *Wallet) ListTransactionDetails(txHash *chainhash.Hash) ([]dosjson.ListTransactionsResult, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		txList = make([]dosjson.ListTransactionsResult, 0, len(sends)+len(receives))
		txList = append(txList, receives...)
		txList = append(txList, sends...)
		return nil
	})
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*dosutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32, addresses map[string]struct{}) ([]*dosjson.ListUnspentResult, error) {
	const op errors.Op = "wallet.ListUnspent"
	var results []*dosjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		scSwitch:
			...
			result := &dosjson.ListUnspentResult{
...
func (w *Wallet) DumpWIFPrivateKey(addr dosutil.Address) (string, error) {
	...
	wif, err := dosutil.NewWIF(privKey, w.chainParams, dosec.SignatureType(privKey.GetType()))
	if err != nil {
		return "", errors.E(op, err)
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *dosutil.WIF) (string, error) {
	...
	if n, err := w.NetworkBackend(); err == nil {
		err := n.LoadTxFilter(context.TODO(), false, []dosutil.Address{addr}, nil)
		if err != nil {
			return "", errors.E(op, err)
		}
	}
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr dosutil.Address) ([]byte, error) {
...
type StakeInfoData struct {
	BlockHeight  int64
	TotalSubsidy dosutil.Amount
	Sdiff        dosutil.Amount
...
func (w *Wallet) StakeInfoPrecise(chainClient *dcrrpcclient.Client) (*StakeInfoData, error) {
	...
	mempoolTicketsFuture := chainClient.GetRawMempoolAsync(dosjson.GRMTickets)
...
func (w *Wallet) LockedOutpoints() []dosjson.TransactionInput {
	locked := make([]dosjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = dosjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr dosutil.Address) error {
...
// address.
func (w *Wallet) TotalReceivedForAddr(addr dosutil.Address, minConf int32) (dosutil.Amount, error) {
	const op errors.Op = "wallet.TotalReceivedForAddr"
	var amount dosutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// SendOutputsUtxo creates and sends payment transactions. It returns the
// transaction hash upon success
func (w *Wallet) SendOutputsUnspents(outputs []*wire.TxOut, account uint32, inputSource txauthor.InputSource, changeSource txauthor.ChangeSource) (*chainhash.Hash, error) {
	const op errors.Op = "wallet.SendOutputsUnspents"
	//relayFee := w.RelayFee()
	//for _, output := range outputs {
	//	err := txrules.CheckOutput(output, relayFee)
	//	if err != nil {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType, additionalPrevScripts map[wire.OutPoint][]byte,
	additionalKeysByAddress map[string]*dosutil.WIF, p2shRedeemScriptsByAddress map[string][]byte) ([]SignatureError, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr dosutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr dosutil.Address) ([]byte, error) {
...
// address.
func (w *Wallet) CreateSignature(tx *wire.MsgTx, idx uint32, addr dosutil.Address, hashType txscript.SigHashType, prevPkScript []byte) (sig, pubkey []byte, err error) {
...
func Open(cfg *Config) (*Wallet, error) {
	...
	w := &Wallet{
		...
		disallowChangeAddress: cfg.DisallowChangeAddress,
	...
	// Amounts
	w.relayFee, err = dosutil.NewAmount(cfg.RelayFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
