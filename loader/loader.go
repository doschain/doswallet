...
import (
	...
	_ "gogs.doschain.org/doschain/doswallet/wallet/drivers/bdb" // driver loaded during init
)
...
func NewLoader(chainParams *chaincfg.Params, dbDirPath string, gapLimit int,
	allowHighFees bool, relayFee float64, accountGapLimit int, disallowChangeAddress bool) *Loader {
	...
	return &Loader{
		...
		disallowChangeAddress: disallowChangeAddress,
...
func (l *Loader) CreateWatchingOnlyWallet(extendedPubKey string, pubPass []byte) (w *wallet.Wallet, err error) {
	...
	cfg := &wallet.Config{
		...
		DisallowChangeAddress: l.disallowChangeAddress,
